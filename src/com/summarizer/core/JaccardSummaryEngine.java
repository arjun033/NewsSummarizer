...
public class JaccardSummaryEngine extends SummaryEngine {
	
	public JaccardSummaryEngine(String corpusPath, String summaryPath) {
	...
	public Map<Integer, Set<Integer>> getAdjacencyList(double[][] similarityMatrix) {
		...
		for(int i=0; i<similarityMatrix.length; i++){
			for(int j=0; j<similarityMatrix[i].length; j++){
				if(i!=j && similarityMatrix[i][j] >= NewsSummarizerApp.jaccardThreshold) {
	...
	public double[][] buildSimilarityMatrix() {
		...
		Similarity sim = SimilarityFactory.getSimilarityInstance(SimilarityType.JACCARD);
		int i=0;
	...
	public void printOutput(Set<Integer> keepNodes) throws IOException{
		...
		String fileName = this.summaryPath+"/Summary_"+Integer.toString(sim)+"_"+SimilarityType.JACCARD.toString()+".txt";
